...
\author[1]{\fnm{T.} \sur{Arzayeva}}
\author*[1]{\fnm{B.} \sur{Aimurzina}}
\author[1]{\fnm{Zh.} \sur{Bulakbay}}
\author[1]{\fnm{G.} \sur{Mataibaeva}}
\author[1]{\fnm{G.} \sur{Kodasheva}}

\affil*[1]{\orgname{L.N. Gumilyov Eurasian National University}, \orgaddress{\city{Astana}, \country{Kazakhstan}}}

\abstract{Women's entrepreneurship in agriculture is recognized as an important factor in inclusive growth; however, empirical evidence from Central Asian transition economies remains limited. This study analyzes the causal influence of institutional agricultural lending on the development of women-owned farms across 20 regions of Kazakhstan during 2015--2024 using 175 observations. The empirical design combines two-way fixed-effects panel regressions, k-means clustering, and Random Forest models. The results show that credit has a positive cross-sectional association with women-led farming in pooled specifications, but does not retain a statistically significant causal effect after regional and time fixed effects are introduced. Regional clustering reveals distinct development paths, while access to land emerges as a binding institutional constraint that limits the effectiveness of financial support. The findings suggest that agricultural policy should move beyond uniform lending expansion and combine financial instruments with land-access and region-specific institutional reforms.}
...
\noindent\textbf{Funding.} This article was prepared within project AP26100879, ``Design and Implementation of an Inclusive Agricultural Development Model,'' supported under the 2025--2027 competition for grant funding for scientific and scientific-technical projects of the Ministry of Science and Higher Education of the Republic of Kazakhstan.

\section{Introduction}\label{sec:introduction}

Women's entrepreneurship in agriculture is increasingly recognized as a key driver of inclusive economic growth, rural development, and food security in both developing and transition economies \citep{ref78}. Amid the transformation of agricultural systems, ensuring women's equal access to land, finance, and technology is becoming not only a matter of social justice but also a prerequisite for raising sectoral productivity \citep{ref62,ref26}.

Kazakhstan retains a substantial state role in agricultural finance through institutions such as KazAgroFinance JSC, the Agrarian Credit Corporation JSC, and the Fund for Financial Support of Agriculture JSC. At the same time, the share of women heading farms remains low, and women's control over agricultural land is especially limited: on average, only 2.01\% of agricultural land is registered to women, with a median of 1.19\%.

The question of how access to finance affects farmers' performance has been widely studied in the literature \citep{ref94,ref99}. However, most empirical studies have at least three limitations. First, they rely on cross-sectional data and therefore do not clearly distinguish correlation from causation. Second, their geographical focus is concentrated in Africa and South Asia, while post-Soviet countries with distinct institutional legacies remain largely outside the evidence base. Third, the interaction between lending and access to land is rarely examined, although this combination may be decisive for women farmers.

This study addresses these gaps by analyzing panel data for 20 regions of Kazakhstan over a 10-year period (2015--2024, 175 observations). The contribution of this work is threefold. First, it identifies the channels through which institutional agricultural lending may influence the development of women-owned farms in the Central Asian context. Second, it applies a three-module analytical design that combines fixed-effects econometrics, cluster analysis, and machine learning to disentangle interregional correlation, intraregional dynamics, and predictive importance. Third, it tests whether access to land acts as a binding institutional constraint that reduces the potential impact of financial support.

Based on the theoretical discussion and gaps in the literature, the study tests three hypotheses. H1: access to institutional agricultural lending has no statistically significant effect on the number of women-owned farms within regions. H2: the relationship between lending and women's entrepreneurial outcomes is heterogeneous and varies with regional infrastructure and institutional capacity. H3: access to land (land\_share) positively moderates the effect of lending, but its institutional limitation reduces the overall effectiveness of financial support.

The empirical strategy is based on an unbalanced panel combining data from the National Statistics Bureau of the Republic of Kazakhstan, state agricultural financial institutions, and the Land Resources Management Committee. We estimate two-way fixed-effects models with regional clustering of standard errors, apply k-means clustering to identify stable regional regimes, and use Random Forest with group cross-validation to assess predictive power.

The remainder of the article is organized as follows. Section 2 reviews the literature and develops the conceptual framework. Section 3 describes the data, variables, and analytical framework. Section 4 presents the empirical results. Section 5 discusses the findings in relation to prior research. Section 6 develops policy recommendations for Kazakhstan's agricultural sector. Section 7 concludes.
...
\citet{ref81} emphasize inclusivity in the value chain.
\citet{ref24} notes that innovations in gender development are key
...
\citep{ref34,ref90}. Others, on the
...
by \citet{ref55}, which identifies five key areas for the
...
\citet{ref47}, \citet{ref64}, and \citet{ref96} note that structural changes in rural
...
Some authors emphasize value chains \citep{ref81}, while
...
However, there is one exception. \citet{ref18} proposes
...
\citet{ref63} and \citet{ref76} identify several factors
...
such as \citep{ref56,ref80}.

The key role of digital finance in promoting the sustainable development
of rural regions is explored in the study by \citet{ref87}.
...
including \citet{ref57}; \citet{ref31}; \citet{ref05}; \citet{ref106}.
...
\citet{ref92} add another important dimension.
...
\citet{ref88}, and \citet{ref16} note that
...
\citet{ref99} demonstrate just how high the potential of
...
But realizing this potential, as noted by \citet{ref25},
...
The systematic logic of the study's design ensures a phased approach to
addressing the following tasks: first, the presence of a causal
relationship is verified through panel econometric analysis
\citep{ref22,ref53,ref101}; second, the structural context of the
phenomenon is revealed through cluster analysis; and third, the
predictive significance of the results is confirmed through machine
learning methods.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image002.png}
...
model in an unbalanced panel sample \citep{ref100}. The
...
log$_i$t = $\beta_0 + \beta_1 \log(\text{credit})_{it} + \beta_2 \log(\text{land})_{it} + \mu_{it}$ (1)
...
$Y_{it} = \alpha_i + \lambda_t + \beta X_{it} + \mu_{it}$ (2)

Where:

$Y_{it}$ -- dependent variable (number of women-owned farms in region $i$ in year $t$)

$\alpha_i$ -- fixed effect of the region (regional-specific characteristics)

$\lambda_t$ -- fixed effect of the year (time-specific effects)

$\mu_{it}$ -- random error term
...
\subsubsection{Module 2: Analysis of regional heterogeneity via clustering}
...
\subsubsection{Module 3: Machine learning for predictive power and non-linearity assessment}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image2.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image3.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image4.png}
\caption{The relationship between lending and women's entrepreneurship in agriculture}
\label{fig:lending-entrepreneurship}
\end{figure}

Figure 4 illustrates this relationship visually. Correlation analysis
...
$R^2$ = 0.336, meaning the model explains 33.6\% of the interregional
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image5.png}
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{0.13\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.19\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}@{}}
\toprule
Cluster & Number of regions & log\_women\_farms (mean value) & log\_credit\_total (mean value) & Share of land (average value, \%) & Growth trend (annual)\\
\midrule
...
rate in percentage terms. For Cluster 0, $\beta$ = 0.091 yields about 9.5\%
growth per year; for Cluster 1, $\beta$ = 0.296 yields about 34.5\% per
year.*}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image6.png}
\caption{Regional clusters based on lending and women's entrepreneurship}
\label{fig:regional-clusters}
\end{figure}

Figure 6 shows the regional cluster analysis results. In the scatter
plot, Cluster 0 (blue) is located in the high-value are
...
\subsection{Analysis of features' importance: Random Forest}
...
\begin{table}[h]
\caption{Feature Importance from the Random Forest Model}\label{tab:feature-importance}%
\small
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image7.png}
\caption{Relationship between loan amount and the number of farms owned by women}
\label{fig:loan-amount-farms}
\end{figure}

The graph generated using the Random Forest model (Figure 7) shows a
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth,keepaspectratio]{../figures/image8.png}
...
hypothesis for entrepreneurial activity described by \citet{ref18}.
...
\begin{enumerate}
\item Lending does not have a negative impact on the growth in the number
...
\item Two clusters of rural development emerge from the analysis.
...
(log\_credit\_total = 8.02). It has relatively higher access to land
for women farmers (2.3\%), but a moderate growth rate of 9.5\% per
year. Cluster 1 (5 regions) exhibits low baseline levels
(log\_women\_farms = 6.35, log\_credit\_total = 6.75), extremely
limited access to land (0.18\%), but growth of approximately 34.5\% per
...
\item The findings have direct implications for Kazakhstan's
...
\item Random Forest analysis confirmed that variables characterizing
...
\subsection{Research results. So, why don't loans work?}
...
\citet{ref43} reaches a similar conclusion in his study of
...
\subsection{Regional characteristics of support policies for women farmers in Kazakhstan}
...
Compared to studies in South Asia \citep{ref44} and Africa
\citep{ref27}, the problem of ``limited access'' due
...
\subsection{Reliability of results: methodological aspect}
...
\begin{itemize}
\item For Cluster 0 regions (developed): The priority is to improve the
efficiency of existing resources through financial literacy programs,
advisory support, and the promotion of cooperation.
\item For Cluster 1 regions (catching up): Comprehensive programs are
needed that combine financial support with institutional reforms,
expanding women's access to land, developing infrastructure, and
