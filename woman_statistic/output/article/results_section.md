...
Данный раздел использует следующие переменные из панельного датасета по 20 регионам Казахстана (2015–2024 гг.):

**Основные показатели:**
- **women_farms** — число сельскохозяйственных МСП и крестьянско-фермерских хозяйств под управлением женщин (количество, шт.)
- **credit_total** — общий объём институционального агрокредитования женщинам (млн тенге), сумма трёх компонентов

**Компоненты агрокредитования:**
- **credit_kaf** — кредиты женщинам через АО «КазАгроФинанс» (KazAgroFinance), млн тенге
- **credit_acc** — кредиты женщинам через АО «Аграрная кредитная корпорация» (Agricultural Credit Corporation), млн тенге
- **credit_fund** — кредиты женщинам через АО «Фонд финансовой поддержки сельского хозяйства» (Fund for Financial Support of Agriculture), млн тенге

**Земельные ресурсы:**
- **land_share** — доля женщин, которым предоставлены земли сельскохозяйственного назначения, %

**Логарифмические трансформации:**
- **log_women_farms** — log(1 + women_farms), логарифмическая трансформация числа женских фермерств
- **log_credit_total** — log(1 + credit_total), логарифмическая трансформация объёма кредитования
- **log_credit_kaf, log_credit_acc, log_credit_fund** — логарифмические трансформации компонентов кредитования

**Лаговые переменные:**
- **lag1_log_credit_total** — лаг первого порядка переменной log_credit_total (значение предыдущего года)
- **lag1_log_women_farms** — лаг первого порядка переменной log_women_farms (значение предыдущего года)

**Идентификаторы:**
- **region_id** — числовой идентификатор региона (1–20)
- **year** — год наблюдения (2015–2024)

*Примечание: Логарифмическая трансформация log(1 + x) применена для нормализации распределения и возможности интерпретации коэффициентов регрессии как эластичностей (процентное изменение зависимой переменной при 1% изменении независимой переменной).*
